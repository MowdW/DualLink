...
import { getCleanLocalPath } from './path-utils';
import {
    ...
    this.registerMarkdownPostProcessor((element, context) => {
      ...
      images.forEach((img) => {
        ...
        if (src && (src.startsWith('local-file://') || src.startsWith('file:///') || src.startsWith('app://local/'))) {
          ...
          if (src.startsWith('app://local/')) {
            const raw = decodeURIComponent(src.replace(/^app:\/\/local\/[^\/]*\//, 'app://local/').replace('app://local/', ''));
            const driveMatch = raw.match(/[a-zA-Z]:\//);
            filePath = driveMatch ? raw.substring(raw.indexOf(driveMatch[0])) : raw;
          } else {
            filePath = getCleanLocalPath(src);
          }
          if (!filePath) return;
      ...
      links.forEach(a => {
        ...
        if (href && (href.startsWith('local-file://') || href.startsWith('file:///') || href.startsWith('app://local/'))) {
          let filePath: string;
          if (href.startsWith('app://local/')) {
            const raw = decodeURIComponent(href.replace(/^app:\/\/local\/[^\/]*\//, 'app://local/').replace('app://local/', ''));
            const driveMatch = raw.match(/[a-zA-Z]:\//);
            filePath = driveMatch ? raw.substring(raw.indexOf(driveMatch[0])) : raw;
          } else {
            filePath = getCleanLocalPath(href);
          }
          if (!filePath) return;
          const ext = filePath.split('.').pop()?.toLowerCase() || '';
